...
- **`frame.time` is corrupted.** 90% of rows retain a clock time, but
...
- **Row order is not capture order.** The file is **15 contiguous blocks**, one per
  attack type, for 15 types.
